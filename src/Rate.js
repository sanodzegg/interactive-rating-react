...
class Rate extends Component {
    ...
    updateScore = (e) => {
      const clicks = e.target.parentNode.children;
      this.setState({rateCircles: clicks});
      [...clicks].forEach(e => {
        if(e.className.includes('active')) {
          e.className = e.className.replace('active', '');
        }
      });
      let newScore = parseInt(e.target.innerText);
      this.setState({score: newScore});
      e.target.className += ' active';
    }

    send = (e) => {
      if(this.state.rateCircles.length <= 0) {
        e.preventDefault();
      } else {
    ...
    render() { 
      if(this.state.sent !== true) {
        return ( 
          <Card className='rounded bg-dark border-0' style={{width: '27.778vw'}}>
            <div className='mb-3 d-flex justify-content-center align-items-center rounded-circle bg-secondary icon'>
              <svg width="17" height="16" xmlns="http://www.w3.org/2000/svg"><path d="m9.067.43 1.99 4.031c.112.228.33.386.58.422l4.45.647a.772.772 0 0 1 .427 1.316l-3.22 3.138a.773.773 0 0 0-.222.683l.76 4.431a.772.772 0 0 1-1.12.813l-3.98-2.092a.773.773 0 0 0-.718 0l-3.98 2.092a.772.772 0 0 1-1.119-.813l.76-4.431a.77.77 0 0 0-.222-.683L.233 6.846A.772.772 0 0 1 .661 5.53l4.449-.647a.772.772 0 0 0 .58-.422L7.68.43a.774.774 0 0 1 1.387 0Z" fill="#FC7614"/></svg>
            </div>
            <Card.Body>
              <Card.Title className='text-light'>How did we do?</Card.Title>
              <Card.Text className='text-muted'>Please let us know how we did with your support request. All feedback is appreciated to help us improve our offering!</Card.Text>
            </Card.Body>
            <div className='rate-circles d-flex justify-content-between text-muted'>
              <div onClick={this.updateScore} role='button' className='circle rounded-circle bg-secondary mb-3 mt-3'>1</div>
              <div onClick={this.updateScore} role='button' className='circle rounded-circle bg-secondary mb-3 mt-3'>2</div>
              <div onClick={this.updateScore} role='button' className='circle rounded-circle bg-secondary mb-3 mt-3'>3</div>
              <div onClick={this.updateScore} role='button' className='circle rounded-circle bg-secondary mb-3 mt-3'>4</div>
              <div onClick={this.updateScore} role='button' className='circle rounded-circle bg-secondary mb-3 mt-3'>5</div>
            </div>
            <Button onClick={this.send} className='bg-primary rounded-pill text-light text-uppercase mt-3 p-2'>
              Submit
            </Button>
          </Card>
        );
        ...
        return (
          <Card className='d-flex flex-column align-items-center rounded bg-dark border-0' style={{width: '27.778vw'}}>
            <div className='mb-3 d-flex justify-content-center align-items-center rounded-circle'>
            ...
            <div className='bg-secondary text-center p-1 px-5 text-display'>
              <span className='text-warning text-nowrap'>You selected {this.state.score} out of 5</span>
            </div>
            <h1 className='text-light mt-4'>Thank You!</h1>
            <p className='text-muted medium mt-2'>We appreciate you taking the time to give a rating. If you ever need more support, don't hesitate to get in touch!</p>
          </Card>
        );
      }
    }
